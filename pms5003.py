...
#
# === Single read ===
# Mode suitable for a cronjob: set SLEEP_BETWEEN_READS
# to -1. The sensor will be awakened before the reading, and
# it will be put at sleep before program exit.
#
# === Endless loop ===
# Set SLEEP_BETWEEN_READS to the acquiring interval (seconds).
...
#   * Read option SLEEP_BETWEEN_READS from configuration file.
#
...
DEFAULTS = {
    ...
    'SLEEP_BETWEEN_READS': '-1'                      # Seconds to sleep before repeating averaged read. Use -1 to exit.
}
...
SLEEP_BETWEEN_READS = config.getint('pms5003', 'SLEEP_BETWEEN_READS')
...
port = serial.Serial(DEVICE, baudrate=9600, timeout=SERIAL_TIMEOUT)
